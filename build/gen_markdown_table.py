...
def gen_markdown_table(file, title, data):
    #fields = [0, 1, 3, 4, 7, lambda rec: 'Yes' if rec[6] else 'No']
    max_len = len(max(data, key=lambda x:len(x)))
    fields = list(range(0, max_len))

    new_data = []
    for i in data:
        ilist = list(i)
        new_data.append(tuple((ilist + max_len * [''])[:max_len]))

    headings = [f"Level{i}" for i in range(0, max_len)]
    #align = [('^', '<') if i % 2 == 0 else ('^', '^') for i in range(0, max_len)]
    align = [('^', '<')] * max_len

    file.write(f'{title}\n\n')
    return table(file, new_data, fields, headings, align)
